...
public abstract class BlockLiquid extends Block
{
    ...
    {
        return worldIn.getBlockState(pos).getBlock().getMaterial() == blockMaterial ? worldIn.getBlockState(pos).getValue(BlockLiquid.LEVEL).intValue() : -1;
    }
    ...
    {
        return hitIfLiquid && state.getValue(BlockLiquid.LEVEL).intValue() == 0;
    }
    ...
    protected Vec3 getFlowVector(IBlockAccess worldIn, BlockPos pos)
    {
        ...
        if (worldIn.getBlockState(pos).getValue(BlockLiquid.LEVEL).intValue() >= 8)
        {
    ...
    public boolean checkForMixing(World worldIn, BlockPos pos, IBlockState state)
    {
        if (blockMaterial == Material.lava)
        {
            ...
            if (flag)
            {
                Integer integer = state.getValue(BlockLiquid.LEVEL);
