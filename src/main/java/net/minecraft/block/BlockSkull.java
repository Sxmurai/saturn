...
public class BlockSkull extends BlockContainer
{
    ...
    protected BlockSkull()
    {
        ...
        setDefaultState(blockState.getBaseState().withProperty(BlockSkull.FACING, EnumFacing.NORTH).withProperty(BlockSkull.NODROP, Boolean.valueOf(false)));
    ...
    {
        return getDefaultState().withProperty(BlockSkull.FACING, placer.getHorizontalFacing()).withProperty(BlockSkull.NODROP, Boolean.valueOf(false));
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player)
    {
        ...
        {
            state = state.withProperty(BlockSkull.NODROP, Boolean.valueOf(true));
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
    {
        if (!worldIn.isRemote)
        {
            if (!state.getValue(BlockSkull.NODROP).booleanValue())
            {
                TileEntity tileentity = worldIn.getTileEntity(pos);
    ...
    {
        return getDefaultState().withProperty(BlockSkull.FACING, EnumFacing.getFront(meta & 7)).withProperty(BlockSkull.NODROP, Boolean.valueOf((meta & 8) > 0));
    }
    ...
    {
        int i = 0;
        i = i | state.getValue(BlockSkull.FACING).getIndex();
    ...
    {
        return new BlockState(this, BlockSkull.FACING, BlockSkull.NODROP);
    }
    ...
    protected BlockPattern getWitherPattern()
    {
        ...
        {
            witherPattern = FactoryBlockPattern.start().aisle("^^^", "###", "~#~").where('#', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.soul_sand))).where('^', BlockSkull.IS_WITHER_SKELETON).where('~', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.air))).build();
        }
