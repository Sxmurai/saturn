    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException
    {
        ...
        else
        {
            ...
            else
            {
                ...
                double d6 = (float) CommandBase.parseDouble(vec3.xCoord, args[1], true);
                double d0 = (float) CommandBase.parseDouble(vec3.yCoord, args[2], true);
                double d1 = (float) CommandBase.parseDouble(vec3.zCoord, args[3], true);
                double d2 = (float) CommandBase.parseDouble(args[4]);
                double d3 = (float) CommandBase.parseDouble(args[5]);
                double d4 = (float) CommandBase.parseDouble(args[6]);
                double d5 = (float) CommandBase.parseDouble(args[7]);
                int i = 0;
                ...
                if (world instanceof WorldServer)
                {
                    ...
                    CommandBase.notifyOperators(sender, this, "commands.particle.success", s, Integer.valueOf(Math.max(i, 1)));
                }
            }
        }
    }
