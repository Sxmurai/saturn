    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException
    {
        if (args.length > 1)
        {
            if (args[0].equals("set"))
            {
                ...
                setTime(sender, l);
                CommandBase.notifyOperators(sender, this, "commands.time.set", Integer.valueOf(l));
                return;
            }

            if (args[0].equals("add"))
            {
                int k = CommandBase.parseInt(args[1], 0);
                addTime(sender, k);
                CommandBase.notifyOperators(sender, this, "commands.time.added", Integer.valueOf(k));
                return;
            }
            ...
            if (args[0].equals("query"))
            {
                if (args[1].equals("daytime"))
                {
                    ...
                    CommandBase.notifyOperators(sender, this, "commands.time.query", Integer.valueOf(j));
                    return;
                }
                ...
                if (args[1].equals("gametime"))
                {
                    ...
                    CommandBase.notifyOperators(sender, this, "commands.time.query", Integer.valueOf(i));
                    return;
                }
            }
        }
