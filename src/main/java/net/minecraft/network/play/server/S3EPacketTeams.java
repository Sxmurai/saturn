...
public class S3EPacketTeams implements Packet<INetHandlerPlayClient>
{
    ...
    public S3EPacketTeams(ScorePlayerTeam p_i45225_1_, int p_i45225_2_)
    {
        ...
        {
            field_149318_b = p_i45225_1_.getTeamName();
            ...
            field_179816_e = p_i45225_1_.getNameTagVisibility().field_178830_e;
            field_179815_f = p_i45225_1_.getChatFormat().getColorIndex();
        }
    ...
    {
        field_149320_a = buf.readStringFromBuffer(16);
        field_149314_f = buf.readByte();

        if (field_149314_f == 0 || field_149314_f == 2)
        {
            field_149318_b = buf.readStringFromBuffer(32);
            field_149319_c = buf.readStringFromBuffer(16);
            field_149316_d = buf.readStringFromBuffer(16);
            field_149315_g = buf.readByte();
            field_179816_e = buf.readStringFromBuffer(32);
            field_179815_f = buf.readByte();
        }
        ...
        if (field_149314_f == 0 || field_149314_f == 3 || field_149314_f == 4)
        {
            ...
            for (int j = 0; j < i; ++j)
            {
                field_149317_e.add(buf.readStringFromBuffer(40));
            }
        }
    }
    ...
    {
        buf.writeString(field_149320_a);
        buf.writeByte(field_149314_f);

        if (field_149314_f == 0 || field_149314_f == 2)
        {
            buf.writeString(field_149318_b);
            buf.writeString(field_149319_c);
            buf.writeString(field_149316_d);
            buf.writeByte(field_149315_g);
            buf.writeString(field_179816_e);
            buf.writeByte(field_179815_f);
        }

        if (field_149314_f == 0 || field_149314_f == 3 || field_149314_f == 4)
        {
            buf.writeVarIntToBuffer(field_149317_e.size());

            for (String s : field_149317_e)
            {
                buf.writeString(s);
            }
        }
