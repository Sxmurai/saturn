package wtf.saturn.util.timing;
...
public class Stopwatch {
    ...
    public boolean hasElapsed(long time, TimeFormat format) {
        switch (format) {
            case SECONDS:
                return passedSeconds(time);
            case MILLISECONDS:
                return passedMs(time);
            case TICKS:
                return passedTicks(time);
            case NANOSECONDS:
                return passedNs(time);
            default:
                return false;
        }
    }
    ...
    public long getTime(TimeFormat format) {
        ...
        switch (format) {
            case SECONDS:
                return elapsedNs / S_TO_MS;
            case MILLISECONDS:
                return elapsedNs / MS_TO_NS;
            case NANOSECONDS:
                return elapsedNs;
            case TICKS:
                return (elapsedNs / MS_TO_NS) / 50L;
            default:
                return -1L;
        }
    }
